...
export class TopicPage implements OnInit {
  ...
  ngOnInit() {
    ...
    } else if (topic == topics.vel_trans) {
      this.topic = new SuperTopic(this.velTrans.name, this.velTrans.formulas);
    } else if (topic == topics.speed) {
      this.topic = new SuperTopic(this.speed.name, this.speed.formulas);
